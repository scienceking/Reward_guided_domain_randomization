...
class RandomizedPendulumEnv(PendulumEnv):
    ...
    def sample_parameters_by_indicator(self, noise_std=0.05):
        ...
        abs_indicator = np.array([abs(indicator) for _, _, indicator in indexed_valid_pairs], dtype=np.float32)
        print(len(abs_indicator))
        temperature = 3
        probs = (abs_indicator + 1e-6) ** temperature
        probs /= probs.sum()
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.time_acc = self.time_acc + end-start

        self.T_time.append(self.time_acc)
        print(len(self.T_time))
